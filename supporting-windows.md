...
Now that doesn't mean this isn't interesting, on the conterary, we'll also experience first hand some of the difficulties of supporting multiple platforms when doing everything from scratch.
{% endhint %}
...
The second part is poorly documented. I've actually struggeled to verify 100% exactly how this will cause a failure on modern Windows but there's enough references to it around from trustworthy sources.

You see, Windows wants to store some information about the currently running thread in what it calls the `Thread Information Block`, referred to as `NT_TIB`. Specifically it wants to store information about the `Stack Base`and the `Stack Limit`in the `%gs`register.

{% hint style="info" %}
What is the GS register you might ask? 

The answer I found was a bit perplexing. Apparently these segment registers, GS on x64, and FS on x86 was intended by Intel to [allow programs to access many different segments of memory](https://stackoverflow.com/questions/10810203/what-is-the-fs-gs-register-intended-for) that were ment to be part of a persistent virtual store. Modern operating systems doesn't use the registers this way as we can only access our own process memory \(which apperas as a "flat" memory to us\). Back when it wasn't clear that this would be the prevailing model, these registers would allow for different implementations by different operating systems. See the [Wikipedia article on the Multics operating system](https://en.wikipedia.org/wiki/Multics) if you're curious.
{% endhint %}
...
As you might notice Windows expects information about the currently running thread, and when we switch threads, we should provide this information. Specifically it wants information about our [Stack Base and our Stack Limit](https://en.wikipedia.org/wiki/Win32_Thread_Information_Block).
...
I renamed the fields `stack_start`and `stack_end`since I find that easier to mentally parse since we know the stack starts on the top and grows downwards to the end.
...
{% hint style="warning" %}
You'll notice I've changed the inline assembly slightly here since I've had some issues with the `=m`constraint and compilition in release mode. This code does the same and our original example will be updated once I figure out a solution to why the code will not run as expected in release builds. 

Meanwhile I'll explain briefly here:

We use the `{register}`constraint here which tells the compiler to put our input variables into a specific register. The `%rdi`and the `%rsi`registers are not randomly chosen, on Linux systems they are the default first and second argument in a function call. Not stricly needed but a nice convention even though Windows has different default registers for arguments \(`%rcx`and `%rdx`\).
...
I've collected all the code we need to compile differently for Windows at the bottom so you don't have to read the first 200 lines of code since that is unchanged from what we in the previous chapters. I hope you understand why I chose to dedicate a seperate chapter for this.
